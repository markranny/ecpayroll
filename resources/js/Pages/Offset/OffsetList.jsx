...
const OffsetList = ({ offsets, onStatusUpdate, onDelete, refreshInterval = 5000 }) => {
    ...
    useEffect(() => {
        ...
        const refreshData = async () => {
            try {
                ...
                if (typeof window.refreshOffsets === 'function') {
                    const freshData = await window.refreshOffsets();
                    setLocalOffsets(freshData);
                    applyFilters(freshData, filterStatus, searchTerm, dateRange);
                }
            } catch (error) {
                console.error('Error refreshing offset data:', error);
            }
        };
    ...
    const applyFilters = (data, status, search, dates) => {
        ...
        if (dates.from && dates.to) {
            result = result.filter(ot => {
                ...
                const offsetDate = new Date(ot.date);
    ...
    const toggleSelectAll = () => {
        ...
        if (!selectAll) {
            // Only select pending offsets
            const pendingIds = filteredOffsets
    ...
    const pendingItemsCount = filteredOffsets.filter(ot => ot.status === 'pending').length;
